...
公共编辑器行为都在 Core,三套 Adapter 只负责各自 UI 库渲染。项目也为 Element Plus、Naive UI、Ant Design Vue 分别保留组件测试,覆盖链接、图片、表格、工具栏配置等高风险交互;表格抓手、合并气泡和菜单密度还通过 `pnpm test:table:e2e` 做三套 UI 浏览器回归。
