...
- （2026-09-07）**CI 受信发布（OIDC）是首选发布路径**：`.github/workflows/release.yml`（手动 `workflow_dispatch` 触发）在 GitHub 托管 runner 上以 `id-token: write` 运行 `scripts/publish-oidc.mjs`——零 token、零 2FA，npm 以 Actions 运行身份直接验证来源，并附 `--provenance` 构建证明；发布后自动跑 `verify-publish.mjs` 消费者验证。启用前置（**每个包单独配置**，npmjs.com → 包 Settings → Trusted publishing）：`twoer / tiptap-vue-pro / release.yml`，environment 留空；workflow 文件名必须逐字一致（含扩展名）。已知坑：配置保存时不校验、错误只在发布时暴露（遇 ENEEDAUTH 先逐字核对文件名）；已建连接不可修改只能删除重建；仅支持 GitHub 托管 runner；`workflow_dispatch` 要求 workflow 文件已存在于默认分支。发布节奏：本地 `changeset version` + 提交推送 main → Actions 页手动点 Release。上面的本地 PTY / 网页认证流程保留为无 CI 时的后备。
