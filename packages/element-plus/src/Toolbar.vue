...
import { ElButton, ElTooltip, ElDropdown, ElDropdownMenu, ElDropdownItem, ElDialog, ElInput, ElCheckbox, ElMessage } from 'element-plus'
import {
...
function exportMarkdown() {
  ...
  if (!md) {
    ElMessage.warning('当前未启用 Markdown 能力,无法导出')
    return
  }
...
function confirmLink() {
  ...
  if (!/^(https?:|mailto:|tel:)/i.test(href) && !/\.[a-z]{2,}/i.test(href)) {
    ElMessage.warning('链接格式不正确,请输入完整网址(如 https://example.com)')
    return
  }
...
<template>
  <div class="tvp-toolbar">
    ...
    <ElTooltip v-if="uploadImage" content="上传图片" placement="top" :show-after="300">
      <ElButton text class="tvp-icon-btn" @click="triggerImageUpload"><ImagePlus :size="18" /></ElButton>
    </ElTooltip>
    <input
      ref="imageInput"
      type="file"
    ...
    <ElTooltip :content="isPreview ? '编辑' : '预览'" placement="top" :show-after="300">
      <ElButton text class="tvp-icon-btn" @click="emit('toggle-preview')">
    ...
    <ElTooltip :content="isFullscreen ? '退出全屏' : '全屏'" placement="top" :show-after="300">
      <ElButton text class="tvp-icon-btn" @click="emit('toggle-fullscreen')">
