...
describe('Element Plus TableGripHandles', () => {
  ...
  it('行列菜单文案保持简化,图标和文字使用统一 6px 间距', () => {
    ...
    for (const text of ['在上方插入', '在下方插入', '上移', '下移', '在左侧插入', '在右侧插入', '左移', '右移', '删除']) {
    ...
    expect(source).toContain('popper-class="tvp-table-grip-dropdown"')
