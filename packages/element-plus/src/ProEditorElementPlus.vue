...
import { MATH_NODE_VIEW_CONTEXT, MERMAID_NODE_VIEW_CONTEXT, SLASH_COMMAND_ITEMS, createDebugLogger, resolveEditorBehaviorOptions, resolveLocale, runSlashCommandItem, useEditorEventBridge, useImageDropPaste, useProEditor, type ActiveMathNode, type AutosaveOptions, type AutosaveState, type LocalDraftCandidate, type LocalDraftOptions, type SlashCommandItem, type SlashCommandRenderState, type UploadAsset, type UploadImage, type OutputFormat, type Extensions, type NotifyType, type ProEditorContext, type ToolbarLayoutMode, type ToolbarOptions, type ToolbarProp, type EditorBehaviorOptions, type LocaleKey, type LocaleProp, type ProEditorDebugLogger, type ProEditorDebugLogFn, type ProEditorDebugOptions } from 'tiptap-vue-pro-core'
...
watch(
  ...
  (state) => {
    if (state.status === 'available' && state.candidate) {
      emit('draft-found', { ...state.candidate })
    }
...
function openMathEdit(math: ActiveMathNode) {
  debugLog('adapter', 'dialog-open', { dialog: 'math-edit', kind: math.kind })
...
function confirmMathEdit(latex: string, kind: 'inline' | 'block' = 'block') {
  ...
  debugLog('adapter', 'dialog-confirm', { dialog: 'math-edit' })
  if (target) {
    if (kind === target.kind) {
  ...
  >
    ...
    >
      ...
      <span class="tvp-draft-recovery__actions">
        <ElButton link size="small" class="tvp-draft-restore" @click="restoreDraft">{{ t('draft.recovery.restore') }}</ElButton>
        <ElButton link size="small" class="tvp-draft-discard" @click="discardDraft">{{ t('draft.recovery.discard') }}</ElButton>
      </span>
    </div>
    ...
    <ElDialog v-model="slashImageUrlDialogVisible" :title="t('toolbar.image.urlDialogTitle')" width="420px" append-to-body>
      <ElInput
      ...
      />
      <template #footer>
        <ElButton @click="slashImageUrlDialogVisible = false">{{ t('toolbar.action.cancel') }}</ElButton>
        <ElButton type="primary" @click="confirmSlashUrlImage">{{ t('toolbar.action.confirm') }}</ElButton>
      </template>
    </ElDialog>
  </div>
</template>
...
.tvp-editor--dark {
  ...
  --el-color-warning-light-9: #2b2517;
  --el-color-warning-light-7: #594a1f;
}
...
.tvp-editor--element-plus .tvp-content .ProseMirror .tvp-find-match {
  border-radius: 2px;
  background: color-mix(in srgb, var(--el-color-primary, #409eff) 22%, transparent);
}

.tvp-editor--element-plus .tvp-content .ProseMirror .tvp-find-match--active {
  background: color-mix(in srgb, var(--el-color-primary, #409eff) 42%, transparent);
...
.tvp-content .ProseMirror hr::before {
  ...
  left: 0;
  right: 0;
  border-top: 1.5px solid var(--el-border-color, #dcdfe6);
...
.tvp-editor--element-plus:not(.tvp-editor--readonly):not(.is-preview) .tvp-content .ProseMirror hr.ProseMirror-selectednode,
.tvp-editor--element-plus:not(.tvp-editor--readonly):not(.is-preview) .tvp-content .ProseMirror hr.tvp-range-selected-node {
  outline: 2px solid var(--el-color-primary-light-5, #a0cfff);
  outline-offset: 4px;
}

/* hover 描边:浅主色提示分割线可点选(只读/预览态不显示,已选中时不重复) */
.tvp-editor--element-plus:not(.tvp-editor--readonly):not(.is-preview) .tvp-content .ProseMirror hr:hover:not(.ProseMirror-selectednode):not(.tvp-range-selected-node) {
  outline: 1.5px solid var(--el-color-primary-light-5, #a0cfff);
  outline-offset: 4px;
}
...
.tvp-editor--element-plus:not(.tvp-editor--readonly):not(.is-preview) .tvp-content .ProseMirror pre.tvp-range-selected-node {
 ...
 */
.tvp-editor--element-plus:not(.tvp-editor--readonly):not(.is-preview) .tvp-content .ProseMirror .tvp-img-node.ProseMirror-selectednode,
.tvp-editor--element-plus:not(.tvp-editor--readonly):not(.is-preview) .tvp-content .ProseMirror .tvp-img-node.tvp-range-selected-node {
  outline: 2px solid var(--el-color-primary, #409eff);
  outline-offset: 2px;
  border-radius: 4px;
}
 ...
 * 只读/预览与已选中时不重复显示。
 */
.tvp-editor--element-plus:not(.tvp-editor--readonly):not(.is-preview) .tvp-content .ProseMirror .tvp-img-node:not(.ProseMirror-selectednode):not(.tvp-range-selected-node):hover {
  outline: 1.5px solid var(--el-color-primary-light-5, #a0cfff);
  outline-offset: 2px;
  border-radius: 4px;
}
...
.tvp-editor--element-plus:not(.tvp-editor--readonly):not(.is-preview) .tvp-content .ProseMirror .tvp-img-node:hover .tvp-img-caption-empty,
.tvp-editor--element-plus:not(.tvp-editor--readonly):not(.is-preview) .tvp-content .ProseMirror .tvp-img-node.ProseMirror-selectednode .tvp-img-caption-empty,
...
/* 选中态描边:只读/预览态不显示,仅保留编辑态的选中反馈 */
.tvp-editor--element-plus:not(.tvp-editor--readonly):not(.is-preview) .tvp-content .ProseMirror .tvp-media-node.ProseMirror-selectednode,
.tvp-editor--element-plus:not(.tvp-editor--readonly):not(.is-preview) .tvp-content .ProseMirror .tvp-media-node.tvp-range-selected-node {
  outline: 2px solid var(--el-color-primary, #409eff);
  outline-offset: 2px;
}

/* hover 描边:浅主色提示媒体节点可点选(只读/预览态不显示,已选中时不重复) */
.tvp-editor--element-plus:not(.tvp-editor--readonly):not(.is-preview) .tvp-content .ProseMirror .tvp-media-node:hover:not(.ProseMirror-selectednode):not(.tvp-range-selected-node) {
  outline: 1.5px solid var(--el-color-primary-light-5, #a0cfff);
  outline-offset: 2px;
}
...
.tvp-editor--element-plus:not(.tvp-editor--readonly):not(.is-preview) .tvp-content .ProseMirror .tvp-file-attachment.ProseMirror-selectednode,
.tvp-editor--element-plus:not(.tvp-editor--readonly):not(.is-preview) .tvp-content .ProseMirror .tvp-file-attachment.tvp-range-selected-node {
  border-color: var(--el-color-primary, #409eff);
  box-shadow: 0 0 0 2px var(--el-color-primary-light-8, #d9ecff);
}
...
.tvp-editor--element-plus .tvp-content .ProseMirror table {
  border-collapse: collapse;
  width: 100%;
  margin: 0.5em 0;
}

.tvp-editor--element-plus .tvp-content .ProseMirror th,
.tvp-editor--element-plus .tvp-content .ProseMirror td {
  position: relative;
  border: 1px solid var(--el-border-color, #dcdfe6);
  padding: 6px 10px;
}
...
.tvp-editor--element-plus .tvp-content .ProseMirror .selectedCell {
  position: relative;
}

.tvp-editor--element-plus .tvp-content .ProseMirror .selectedCell::after {
  content: '';
  position: absolute;
  inset: 0;
  pointer-events: none;
...
.tvp-editor--element-plus .tvp-content .ProseMirror.resize-cursor {
  cursor: col-resize;
}

.tvp-editor--element-plus .tvp-content .ProseMirror .column-resize-handle {
  position: absolute;
  top: -1px;
  right: -3px;
  bottom: -1px;
...
.tvp-editor--element-plus .tvp-content .ProseMirror .column-resize-handle::before {
  content: '';
  position: absolute;
  top: 0;
  bottom: 0;
...
.tvp-draft-recovery {
  ...
  border-bottom: 1px solid var(--el-color-warning-light-7, #f3d19e);
  font-size: 12px;
}
...
.tvp-editor--element-plus .tvp-draft-recovery .tvp-draft-restore,
.tvp-editor--element-plus .tvp-draft-recovery .tvp-draft-discard {
