...
<template>
  <div ref="rootEl" class="tvp-file-bubble">
    <AntTooltip :content="t('file.action.edit')" placement="top" :show-after="300">
      <AntButton text :aria-label="t('file.action.edit')" @click="openEditDialog">
        <Pencil :size="16" />
      </AntButton>
    </AntTooltip>
    <AntTooltip :content="t('file.action.open')" placement="top" :show-after="300">
      <AntButton text :aria-label="t('file.action.open')" @click="openFile">
        <ExternalLink :size="16" />
      </AntButton>
    </AntTooltip>
    <AntTooltip v-if="canDownload" :content="t('file.action.download')" placement="top" :show-after="300">
      <AntButton text :aria-label="t('file.action.download')" @click="downloadFile">
        <Download :size="16" />
      </AntButton>
    </AntTooltip>
    <AntTooltip :content="t('file.action.copy')" placement="top" :show-after="300">
      <AntButton text :aria-label="t('file.action.copy')" @click="copyFileUrl">
        <Copy :size="16" />
      </AntButton>
    </AntTooltip>
    <AntTooltip v-if="canSwitchToPlayer" :content="t('media.action.asPlayer')" placement="top" :show-after="300">
      <AntButton text :aria-label="t('media.action.asPlayer')" @click="switchToPlayer">
        <PlaySquare :size="16" />
      </AntButton>
    </AntTooltip>
    <AntDivider direction="vertical" />
    <AntTooltip :content="t('file.action.delete')" placement="top" :show-after="300">
      <AntButton text :aria-label="t('file.action.delete')" @click="deleteFile">
        <Trash2 :size="16" />
      </AntButton>
    </AntTooltip>
    ...
    >
      <div class="tvp-file-form">
        <div class="tvp-file-form__row">
          ...
          <AntInput v-model="fileName" clearable />
        </div>
        <div class="tvp-file-form__row">
          <label class="tvp-file-form__label">{{ t('file.typeLabel') }}</label>
          <AntInput v-model="fileTypeText" clearable />
        </div>
        <div class="tvp-file-form__grid">
          <AntCheckbox v-model="showIcon">{{ t('file.showIcon') }}</AntCheckbox>
          <AntCheckbox v-model="showName">{{ t('file.showName') }}</AntCheckbox>
          <AntCheckbox v-model="showSize">{{ t('file.showSize') }}</AntCheckbox>
          <AntCheckbox v-model="showMimeType">{{ t('file.showMimeType') }}</AntCheckbox>
          <AntCheckbox v-model="showUploadedAt">{{ t('file.showUploadedAt') }}</AntCheckbox>
          <AntCheckbox v-model="showDuration">{{ t('file.showDuration') }}</AntCheckbox>
          <AntCheckbox v-model="openInNewTab">{{ t('file.openInNewWindow') }}</AntCheckbox>
          <AntCheckbox v-model="allowDownload">{{ t('file.allowDownload') }}</AntCheckbox>
        </div>
      </div>
      <template #footer>
