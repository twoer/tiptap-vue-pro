...
async function clickBodyText(text: string) {
  ...
  const target = (node!.closest('.tvp-ant-dropdown-menu__item') as HTMLElement | null) ?? node!
...
describe('Ant Design Vue Toolbar', () => {
  ...
  it('工具栏 UI 角色使用 Ant Design Vue 对应控件', () => {
    const source = readFileSync(`${process.cwd()}/src/Toolbar.vue`, 'utf8')

    for (const item of ['heading', 'fontFamily', 'fontSize', 'lineHeight', 'color', 'highlight', 'align']) {
      expect(source).toContain(`<AntDropdown v-else-if="item === '${item}'"`)
    }
    for (const item of ['codeBlock', 'table', 'markdown']) {
      expect(source).toContain(`<AntTooltip v-else-if="item === '${item}'"`)
      expect(source).toMatch(new RegExp(`item === '${item}'[\\s\\S]*<AntDropdown`))
    }
    expect(source).toContain('<AntModal v-model="urlDialogVisible"')
