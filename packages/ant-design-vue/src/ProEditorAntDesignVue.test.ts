...
import ProEditorAntDesignVue from './ProEditorAntDesignVue.vue'
...
describe('ProEditorAntDesignVue', () => {
  ...
  it('在组件实例公开 autosave flush 和 retry 命令', async () => {
    const ctx = createCtx()
    mockState.ctx = ctx
    wrapper = mount(ProEditorAntDesignVue, { global: { stubs: childStubs } })
  ...
  it('显示并执行本地草稿恢复和删除动作', async () => {
    ...
    mockState.ctx = ctx
    wrapper = mount(ProEditorAntDesignVue, { props: { draft: { key: 'article' } }, global: { stubs: childStubs } })
  ...
  it('contextual bubble 根节点默认不可见,避免插件接管前裸露在文档流中', () => {
    const editorSource = readFileSync('src/ProEditorAntDesignVue.vue', 'utf8')
