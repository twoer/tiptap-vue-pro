...
import { AntButton, AntDropdown, AntDropdownMenu, AntDropdownItem, AntTooltip, AntIcon } from './antDesignPrimitives'
import {
  Combine, Split,
...
<template>
  <div ref="rootEl" class="tvp-table-bubble" :style="bubbleStyle">
    ...
    <AntTooltip v-if="bubbleVisible && tableState.canMerge" content="合并单元格" placement="top" :show-after="400">
      <AntButton text class="tvp-icon-btn" @click="ctx.commands.mergeCells()"><Combine :size="15" /></AntButton>
    </AntTooltip>
    <AntTooltip v-if="bubbleVisible && tableState.canSplit" content="拆分单元格" placement="top" :show-after="400">
      <AntButton text class="tvp-icon-btn" @click="ctx.commands.splitCell()"><Split :size="15" /></AntButton>
    </AntTooltip>
    ...
    <AntDropdown
      ...
      trigger="click"
      overlayClassName="tvp-table-bubble-dropdown"
      @command="run"
    >
      <AntButton text class="tvp-icon-btn tvp-table-bubble__more">
        <AntIcon><TableProperties :size="15" /></AntIcon>
      </AntButton>
      <template #dropdown>
        <AntDropdownMenu>
          <AntDropdownItem v-for="item in moreOps" :key="item.op" :command="item.op">
 ...
 * 用 :deep() 穿透 scoped 作用域,命中 AntButton 生成的 .tvp-ant-button。
 * 干掉按钮相邻选择器带来的默认间距,改由容器 gap 统一控制间距。
 */
.tvp-table-bubble :deep(.tvp-ant-button) {
  width: 32px;
  height: 32px;
  padding: 0;
  margin-left: 0;
...
:global(.tvp-table-bubble-dropdown .ant-dropdown-menu) {
  min-width: 168px;
  padding: 6px 0;
}

:global(.tvp-table-bubble-dropdown .tvp-ant-dropdown-menu__item) {
  height: 50px;
  min-height: 50px;
  padding: 0 20px;
  color: var(--tvp-ant-text-color, rgba(0, 0, 0, 0.88));
  font-size: 14px;
  line-height: 22px;
}
