...
<template>
  ...
  >
    <div v-if="view.editable.value" class="tvp-mermaid-toolbar">
      ...
      <NButtonGroup class="tvp-mermaid-mode-group" :aria-label="view.t('mermaid.view.label')">
        <NTooltip v-for="mode in MERMAID_VIEW_MODES" :key="mode" placement="top" :show-arrow="false">
          <template #trigger>
            <NButton
              size="small"
...
<style scoped>
.tvp-mermaid-block {
  --tvp-mermaid-border: #d9d9df;
  --tvp-mermaid-divider: #e7e7ec;
  --tvp-mermaid-surface: #fff;
  --tvp-mermaid-subtle: #f7f7fa;
  --tvp-mermaid-muted: #767c82;
  --tvp-mermaid-text: #26282a;
  --tvp-mermaid-code-bg: #fafafd;
  --tvp-mermaid-code-text: #26282a;
  --tvp-mermaid-code-caret: #18a058;
  --tvp-mermaid-gutter-bg: #f4f4f7;
  --tvp-mermaid-gutter-text: #9a9da1;
  --tvp-mermaid-active-line: rgb(24 160 88 / 8%);
...
.tvp-mermaid-mode-group :deep(.n-button) { width: 28px; height: 28px; padding: 0; }
