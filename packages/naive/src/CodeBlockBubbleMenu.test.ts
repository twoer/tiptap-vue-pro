...
describe('Naive CodeBlockBubbleMenu', () => {
  ...
  it('可以切换语言并复制代码', async () => {
    ...
    const vm = wrapper.vm as unknown as {
      codeBlockOptions: Array<{ key: string }>
      renderCodeBlockLabel: (option: { key: string }) => VNode
      setLanguage: (key: string | number) => void
    }
    ...
    expect(typeScriptLabel.props?.style).toMatchObject({
      display: 'inline-flex',
      alignItems: 'center',
      gap: '6px',
    })
    expect((typeScriptLabel.children as VNode[])[0]?.props?.style).toMatchObject({
      width: '15px',
      height: '15px',
      flex: '0 0 auto',
    })
    expect((typeScriptLabel.children as VNode[])[0]?.props?.viewBox).toBe('0 0 32 32')
    await wrapper.find('button[aria-label="代码语言"]').trigger('click')
    await nextTick()
    expect(document.body.querySelector('.tvp-naive-code-language-dropdown.n-dropdown-menu--scrollable')).toBeTruthy()
    vm.setLanguage('typescript')
