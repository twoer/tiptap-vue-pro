...
const mathFlowStubs = {
  ...
  MathEditDialog: {
    ...
    // 真实弹层 confirm 时同时发出 confirm 与 update:show(false) 自行关闭
    template: `
      <div v-if="show" data-testid="math-edit-dialog-live">
        <span data-testid="math-dialog-kind">{{ math ? math.kind : 'insert' }}</span>
        <button data-testid="math-confirm-block" @click="$emit('confirm', 'x+1', 'block'); $emit('update:show', false)" />
        <button data-testid="math-confirm-inline" @click="$emit('confirm', 'x+1', 'inline'); $emit('update:show', false)" />
        <button data-testid="math-cancel" @click="$emit('update:show', false)" />
      </div>
    `,
  },
}
...
describe('ProEditorNaive', () => {
  ...
  it('contextual bubble 根节点默认不可见,避免插件接管前裸露在文档流中', () => {
    const editorSource = readFileSync('src/ProEditorNaive.vue', 'utf8')
